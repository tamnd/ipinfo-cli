...
describes what users get. Pick a format with `-o`, or let ipinfo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ipinfo <command> -o table   # aligned columns for reading
ipinfo <command> -o jsonl   # one JSON object per line, for piping
ipinfo <command> -o json    # a single JSON array
ipinfo <command> -o csv     # spreadsheet friendly
ipinfo <command> -o tsv     # tab-separated
ipinfo <command> -o url     # just the URL column
ipinfo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ipinfo <command> --fields id,title,url
```
...
```bash
ipinfo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ipinfo <command>            # a table, because this is a terminal
ipinfo <command> | wc -l    # JSONL, because this is a pipe
```
