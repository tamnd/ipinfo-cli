// Package cli assembles the ipinfo command tree from the ipinfo
...
// NewApp assembles the kit application from the ipinfo domain. The
...
//
// To add a command, declare it in ipinfo/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := ipinfo.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(ipinfo.Domain{}).Register(app)
